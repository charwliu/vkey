...
static int register_buildKeys(const unsigned char* u_ILK,const unsigned char* u_IMK,const char* s_url,const char* s_rpk, char* s_ipk,unsigned char* u_isk,char* s_pid,char* s_suk,char* s_vuk);
...
static int register_recover(const unsigned char* u_IUKOLD,const unsigned char* u_IMKOLD,const unsigned char* u_ILKNEW,const unsigned char* u_IMKNEW, const char* s_PID,const char* s_URL,const char* s_RPK)
{
    ...
    unsigned char oldSeedISK[32];
    unsigned char ISKOLD[VKEY_SIG_SK_SIZE];
    unsigned char IPKOLD[VKEY_KEY_SIZE];
    encrypt_hmac(u_IMKOLD,s_URL,strlen(s_URL),oldSeedISK);
    encrypt_makeSignPublic(oldSeedISK,ISKOLD,IPKOLD);
    char strIPKOld[65];
    ...
    char strSUK[65];
    char strVUK[65];
    if(0!=register_buildKeys(u_ILKNEW,u_IMKNEW,s_URL,s_RPK,strIPK,ISK,strPID,strSUK,strVUK))
    {
        return -1;
    }
...
static int register_buildKeys(const unsigned char* u_ILK,const unsigned char* u_IMK,const char* s_url,const char* s_rpk, char* s_ipk,unsigned char* u_isk,char* s_pid,char* s_suk,char* s_vuk)
{
    ...
    encrypt_hmac(u_IMK,s_url,strlen(s_url),seedISK);
    encrypt_makeSignPublic(seedISK,u_isk,IPK);
...
int register_create(const char* s_url,const char* s_rpk, char* s_ipk, unsigned char* u_isk)
{
    ...
    char strSUK[65];
    char strVUK[65];
    if(0!=register_buildKeys(ILK,IMK,s_url,s_rpk,s_ipk,u_isk,strPID,strSUK,strVUK))
    {
        return -1;
    }
...
int register_start(const char* s_url, char* s_rpk)
{
    ...
    if(0==register_getKeys(s_url,RPK,RSK))
    {
    ...
    else
    {
        ...
        encrypt_makeSignPublic(RANDOM, RSK, RPK);

        ///test{{
        unsigned char RID[32]="1233333";
...
//        sqlite3_bind_text(pStmt, 3, strPK, strlen(strPK), SQLITE_TRANSIENT);
        sqlite3_bind_blob(pStmt, 2, RSK, VKEY_SIG_SK_SIZE, SQLITE_TRANSIENT);
        sqlite3_bind_blob(pStmt, 3, RPK, VKEY_KEY_SIZE, SQLITE_TRANSIENT);
        sqlite3_bind_int(pStmt, 4, nTime);
...
//        char *strRSK = (char *) sqlite3_column_text(pStmt, 1);
        unsigned char *uRPK = (unsigned char *) sqlite3_column_blob(pStmt, 0);
        unsigned char *uRSK = (unsigned char *) sqlite3_column_blob(pStmt, 1);
        //todo: descrypt data

        memcpy(u_rpk,uRPK,VKEY_KEY_SIZE);
        memcpy(u_rsk,uRSK,VKEY_SIG_SK_SIZE);
//        strcpy(s_rpk,strRPK);
//        strcpy(s_rsk,strRSK);
        sqlite3_finalize(pStmt);
        return 0;
